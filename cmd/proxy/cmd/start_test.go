	...
	if len(chunks) > 0 {
		...
		if lastChunk.Choices[0].FinishReason != "" {
			if lastChunk.Choices[0].Delta != nil {
				// If delta exists in finish chunk, it should not have empty role
				if lastChunk.Choices[0].Delta.Role == "" && lastChunk.Choices[0].Delta.Content == "" {
					t.Error("Finish chunk should not have delta with empty role and content - delta should be nil")
				}
			}
		}
	}
...
// Test that streaming finish chunks don't include delta with empty role
func TestStreamingFinishChunkDeltaHandling(t *testing.T) {
	// Create a completion response that simulates the GitHub Copilot API sending
	// both content and finish_reason in the same chunk
		...
		Choices: []pkg.Choice{
			{
				...
				FinishReason: pkg.FinishReasonStop, // Both content AND finish_reason
			},
		},
	}
	...
	if response.Choices[0].FinishReason != "" && 
	   ...
	   response.Choices[0].Delta.Content != "" {
		...
		if strings.Contains(jsonStr, `"role":""`) {
			t.Error("Finish chunk JSON should not contain empty role field")
		}
		if strings.Contains(jsonStr, `"content":""`) {
			t.Error("Finish chunk JSON should not contain empty content field")
		}
	}
}
